...
module.exports = (context, student, teacher) => {
  ...
  let schools = getSchools(Number.isInteger(Number.parseInt(unitId)) ? { schoolId: unitId } : { shortName: unitId })
  if (schools.length === 0) schools = getSchools({ schoolNumber: unitId })
  const school = schools.length > 0 ? schools[0] : undefined

  if (!school) {
    logger('warn', ['pifu-api', 'repack-student', 'no school found in vtfk-schools-info for unitId', unitId, student.username])
  } else {
